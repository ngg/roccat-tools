...
#include "roccat.h"
#include "koneplus.h"
#include "koneplus_dbus_plain_services.h"
...
DBusGProxy *koneplus_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEPLUS_DBUS_SERVER_PATH,
			KONEPLUS_DBUS_SERVER_INTERFACE);
...
gboolean koneplus_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Koneplus_profile_data_changed_outside, profile_number);
}
...
gboolean koneplus_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = koneplus_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean koneplus_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Koneplus_profile_changed_outside, profile_number);
}
...
gboolean koneplus_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = koneplus_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean koneplus_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Koneplus_configuration_changed_outside);
}
