...
#define KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_TYPE, KonepureopticalconfigConfigurationDialogClass))
#define IS_KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_TYPE))
#define KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_TYPE, KonepureopticalconfigConfigurationDialogPrivate))

typedef struct _KonepureopticalconfigConfigurationDialogClass KonepureopticalconfigConfigurationDialogClass;

struct _KonepureopticalconfigConfigurationDialogClass {
	GtkDialogClass parent_class;
};

struct _KonepureopticalconfigConfigurationDialogPrivate {
...
G_DEFINE_TYPE(KonepureopticalconfigConfigurationDialog, konepureopticalconfig_configuration_dialog, GTK_TYPE_DIALOG);

static GtkWidget *notification_frame_new(KonepureopticalconfigConfigurationDialog *konepureopticalconfig_configuration_dialog) {
	KonepureopticalconfigConfigurationDialogPrivate *priv = konepureopticalconfig_configuration_dialog->priv;
...
static GtkWidget *general_frame_new(KonepureopticalconfigConfigurationDialog *konepureopticalconfig_configuration_dialog) {
	KonepureopticalconfigConfigurationDialogPrivate *priv = konepureopticalconfig_configuration_dialog->priv;
...
gboolean konepureopticalconfig_configuration_dialog(GtkWindow *parent, RoccatKeyFile *config) {
	KonepureopticalconfigConfigurationDialog *dialog;
	gboolean retval = TRUE;

	dialog = KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG(konepureopticalconfig_configuration_dialog_new(parent));

	konepureopticalconfig_configuration_dialog_set_configuration(dialog, config);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		konepureopticalconfig_configuration_dialog_get_configuration(dialog, config);
		if (!konepureopticalconfig_configuration_save(parent, config))
			retval = FALSE;
	}
...
GtkWidget *konepureopticalconfig_configuration_dialog_new(GtkWindow *parent) {
	KonepureopticalconfigConfigurationDialog *konepureopticalconfig_configuration_dialog;

	konepureopticalconfig_configuration_dialog = g_object_new(KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_TYPE, NULL);

	gtk_dialog_add_buttons(GTK_DIALOG(konepureopticalconfig_configuration_dialog),
			GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
			GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			NULL);
	gtk_window_set_title(GTK_WINDOW(konepureopticalconfig_configuration_dialog), _("Edit preferences"));
	gtk_window_set_transient_for(GTK_WINDOW(konepureopticalconfig_configuration_dialog), parent);
	gtk_window_set_modal(GTK_WINDOW(konepureopticalconfig_configuration_dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(konepureopticalconfig_configuration_dialog), TRUE);
	gtk_window_set_type_hint(GTK_WINDOW(konepureopticalconfig_configuration_dialog), GDK_WINDOW_TYPE_HINT_NORMAL);

	return GTK_WIDGET(konepureopticalconfig_configuration_dialog);
}

static void konepureopticalconfig_configuration_dialog_init(KonepureopticalconfigConfigurationDialog *konepureopticalconfig_configuration_dialog) {
	KonepureopticalconfigConfigurationDialogPrivate *priv = KONEPUREOPTICALCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(konepureopticalconfig_configuration_dialog);
	GtkVBox *content_area;

	konepureopticalconfig_configuration_dialog->priv = priv;

	content_area = GTK_VBOX(gtk_dialog_get_content_area(GTK_DIALOG(konepureopticalconfig_configuration_dialog)));

	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(konepureopticalconfig_configuration_dialog), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(content_area), general_frame_new(konepureopticalconfig_configuration_dialog), TRUE, TRUE, 0);

	gtk_widget_show_all(GTK_WIDGET(content_area));
}

static void konepureopticalconfig_configuration_dialog_class_init(KonepureopticalconfigConfigurationDialogClass *klass) {
	g_type_class_add_private(klass, sizeof(KonepureopticalconfigConfigurationDialogPrivate));
}

gboolean konepureopticalconfig_configuration_save(GtkWindow *parent, RoccatKeyFile *config) {
	GError *error = NULL;

	konepureoptical_configuration_save(config, &error);
