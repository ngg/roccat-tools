...
#include "konepureoptical_dbus_plain_services.h"
...
DBusGProxy *konepureoptical_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEPUREOPTICAL_DBUS_SERVER_PATH,
			KONEPUREOPTICAL_DBUS_SERVER_INTERFACE);
...
gboolean konepureoptical_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepureoptical_profile_data_changed_outside, profile_number);
}
...
gboolean konepureoptical_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = konepureoptical_dbus_emit_profile_data_changed_outside(proxy, profile_number);
...
gboolean konepureoptical_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepureoptical_profile_changed_outside, profile_number);
}
...
gboolean konepureoptical_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = konepureoptical_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean konepureoptical_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Konepureoptical_configuration_changed_outside);
}
