...
#include "konepuremilitary_dbus_plain_services.h"
...
DBusGProxy *konepuremilitary_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEPUREMILITARY_DBUS_SERVER_PATH,
			KONEPUREMILITARY_DBUS_SERVER_INTERFACE);
...
gboolean konepuremilitary_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepuremilitary_profile_data_changed_outside, profile_number);
}
...
gboolean konepuremilitary_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = konepuremilitary_dbus_emit_profile_data_changed_outside(proxy, profile_number);
...
gboolean konepuremilitary_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepuremilitary_profile_changed_outside, profile_number);
}
...
gboolean konepuremilitary_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = konepuremilitary_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean konepuremilitary_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Konepuremilitary_configuration_changed_outside);
}
