...
static gboolean update_firmware(RoccatDevice *device, gchar const *path, GError **error) {
	...
	state = roccat_firmware_state_new(device, KONEPUREMILITARY_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT);
...
int main(int argc, char **argv) {
	...
	if (parameter_just_update_firmware) {
		if (!update_firmware(device, parameter_just_update_firmware, &local_error)) {
