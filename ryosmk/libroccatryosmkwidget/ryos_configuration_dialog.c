...
static GtkWidget *notification_frame_new(RyosConfigurationDialog *ryos_configuration_dialog) {
	...
	gtk_table_attach(GTK_TABLE(table), gtk_label_new(_("Timer")), 0, 1, 0, 1, GTK_FILL, GTK_EXPAND, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), gtk_label_new(_("Profile")), 0, 1, 1, 2, GTK_FILL, GTK_EXPAND, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), gtk_label_new(_("Live recording")), 0, 1, 2, 3, GTK_FILL, GTK_EXPAND, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), gtk_label_new(_("Volume")), 0, 1, 3, 4, GTK_FILL, GTK_EXPAND, 0, 0);
...
void ryos_configuration_dialog_get_configuration(RyosConfigurationDialog *ryos_configuration_dialog, RoccatKeyFile *config) {
	...
	ryos_configuration_set_profile_notification_type(config, roccat_notification_type_combo_box_get_value(priv->profile_notification_type));
	ryos_configuration_set_timer_notification_type(config, roccat_notification_type_combo_box_get_value(priv->timer_notification_type));
	ryos_configuration_set_live_recording_notification_type(config, roccat_notification_type_combo_box_get_value(priv->live_recording_notification_type));
	...
	ryos_configuration_set_notification_volume(config, roccat_volume_scale_get_value(priv->notification_volume));
...
void ryos_configuration_dialog_set_configuration(RyosConfigurationDialog *ryos_configuration_dialog, RoccatKeyFile *config) {
	...
	roccat_volume_scale_set_value(priv->notification_volume, ryos_configuration_get_notification_volume(config));
...
static void ryos_configuration_dialog_init(RyosConfigurationDialog *ryos_configuration_dialog) {
	...
	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(ryos_configuration_dialog), TRUE, TRUE, 0);
