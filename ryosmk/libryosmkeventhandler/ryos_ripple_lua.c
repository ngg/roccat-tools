...
#define RYOS_RIPPLE_LUA_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), RYOS_RIPPLE_LUA_TYPE, RyosRippleLuaClass))
#define IS_RYOS_RIPPLE_LUA_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), RYOS_RIPPLE_LUA_TYPE))

typedef struct _RyosRippleLuaClass RyosRippleLuaClass;

struct _RyosRippleLuaClass {
...
static void ryos_ripple_lua_start(RyosRipple *self, RoccatDevice *device_arg, guint hid) {
	gaminggear_mutex_lock(&mutex);
	device = device_arg;
	memset(&custom_lights.data, 0, RYOS_CUSTOM_LIGHTS_DATA_LENGTH);
	lua_call_start(hid);
...
static void ryos_ripple_lua_interface_init(RyosRippleInterface *iface) {
	iface->start = ryos_ripple_lua_start;
}

G_DEFINE_TYPE_WITH_CODE(RyosRippleLua, ryos_ripple_lua, G_TYPE_OBJECT,
		G_IMPLEMENT_INTERFACE(RYOS_RIPPLE_TYPE, ryos_ripple_lua_interface_init));

RyosRippleLua *ryos_ripple_lua_new(gchar const *module_name) {
	return RYOS_RIPPLE_LUA(g_object_new(RYOS_RIPPLE_LUA_TYPE,
			"module", module_name,
			NULL));
}

static void ryos_ripple_lua_init(RyosRippleLua *ripple_lua) {}

static GObject *ryos_ripple_lua_constructor(GType gtype, guint n_properties, GObjectConstructParam *properties) {
	GError *local_error = NULL;
	GObject *obj;
	gchar *module;
	int retval;

	obj = G_OBJECT_CLASS(ryos_ripple_lua_parent_class)->constructor(gtype, n_properties, properties);
	...
	module = g_build_filename(RYOSMK_RIPPLE_MODULES_PATH, module_name, NULL);
	...
	thread = gaminggear_thread_try_new("RyosmkRipple", worker, NULL, &local_error);
	if (local_error) {
...
static void ryos_ripple_lua_finalize(GObject *object) {
	if (thread)
	...
	G_OBJECT_CLASS(ryos_ripple_lua_parent_class)->finalize(object);
}

static void ryos_ripple_lua_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec) {
	switch(prop_id) {
	case PROP_MODULE:
...
static void ryos_ripple_lua_class_init(RyosRippleLuaClass *klass) {
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS(klass);

	gobject_class->constructor = ryos_ripple_lua_constructor;
	gobject_class->set_property = ryos_ripple_lua_set_property;
	gobject_class->finalize = ryos_ripple_lua_finalize;
