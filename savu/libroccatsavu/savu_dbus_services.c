...
#include "roccat.h"
#include "savu.h"
#include "savu_dbus_plain_services.h"
...
DBusGProxy *savu_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(
			...
			SAVU_DBUS_SERVER_PATH,
			SAVU_DBUS_SERVER_INTERFACE);
...
gboolean savu_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Savu_profile_data_changed_outside, profile_number);
}
...
gboolean savu_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = savu_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean savu_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Savu_profile_changed_outside, profile_number);
}
...
gboolean savu_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = savu_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean savu_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Savu_configuration_changed_outside);
}
