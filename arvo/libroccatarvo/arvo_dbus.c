...
#include "roccat.h"
#include "arvo.h"
#include "arvo_dbus_plain_services.h"
...
DBusGProxy *arvo_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			ARVO_DBUS_SERVER_PATH,
			ARVO_DBUS_SERVER_INTERFACE);
...
gboolean arvo_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Arvo_profile_data_changed_outside, profile_number);
}
...
gboolean arvo_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	proxy = arvo_dbus_proxy_new();
	if (!proxy)
		return FALSE;

	retval = arvo_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean arvo_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Arvo_profile_changed_outside, profile_number);
}
...
gboolean arvo_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = arvo_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean arvo_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Arvo_configuration_changed_outside);
}
