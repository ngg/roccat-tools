...
#include "ryostkl_dbus_plain_services.h"
...
DBusGProxy *ryostkl_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(
			...
			RYOSTKL_DBUS_SERVER_PATH,
			RYOSTKL_DBUS_SERVER_INTERFACE);
...
gboolean ryostkl_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_index) {
	return dbus_roccat_call_y(proxy, org_roccat_Ryostkl_profile_data_changed_outside, profile_index);
}
...
gboolean ryostkl_dbus_emit_profile_data_changed_outside_instant(guint profile_index) {
	...
	retval = ryostkl_dbus_emit_profile_data_changed_outside(proxy, profile_index);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean ryostkl_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_index) {
	return dbus_roccat_call_y(proxy, org_roccat_Ryostkl_profile_changed_outside, profile_index);
}
...
gboolean ryostkl_dbus_emit_profile_changed_outside_instant(guint profile_index) {
	...
	retval = ryostkl_dbus_emit_profile_changed_outside(proxy, profile_index);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean ryostkl_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Ryostkl_configuration_changed_outside);
}
...
gboolean ryostkl_dbus_emit_configuration_changed_outside_instant(void) {
	...
	retval = ryostkl_dbus_emit_configuration_changed_outside(proxy);
	dbus_roccat_proxy_free(proxy);
	return retval;
}
