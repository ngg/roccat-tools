...
#include "roccat.h"
#include "iskufx.h"
#include "iskufx_dbus_plain_services.h"
...
gboolean iskufx_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Iskufx_profile_data_changed_outside, profile_number);
}
...
gboolean iskufx_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = iskufx_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean iskufx_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Iskufx_profile_changed_outside, profile_number);
}
...
gboolean iskufx_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = iskufx_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean iskufx_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Iskufx_configuration_changed_outside);
}
