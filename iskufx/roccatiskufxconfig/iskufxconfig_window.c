...
G_DEFINE_TYPE(IskufxconfigWindow, iskufxconfig_window, ROCCAT_CONFIG_WINDOW_TYPE);
...
static void brightness_changed_from_device_cb(DBusGProxy *proxy, guchar profile_number, guchar brightness_number, gpointer user_data) {
	RoccatConfigWindow *roccat_window = ROCCAT_CONFIG_WINDOW(user_data);
	iskufxconfig_profile_page_update_brightness(ISKUFXCONFIG_PROFILE_PAGE(roccat_config_window_get_page(roccat_window, profile_number - 1)), brightness_number - 1);
}
...
static void macro_changed_from_device_cb(DBusGProxy *proxy, guchar profile_number, guchar button_number, gchar const *macro, gpointer user_data) {
	...
	if (length == sizeof(IskuRkpMacroKeyInfo))
		iskufxconfig_profile_page_update_key(ISKUFXCONFIG_PROFILE_PAGE(roccat_config_window_get_page(roccat_window, profile_number - 1)), button_number - 1, key_info);
	g_free(key_info);
}
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	...
	for (i = 0; i < ISKU_PROFILE_NUM; ++i) {
		profile_page = ISKUFXCONFIG_PROFILE_PAGE(roccat_config_window_get_page(roccat_window, i));
...
static void set_rkp(IskufxconfigWindow *window, guint profile_index, IskuRkp *rkp) {
	IskufxconfigProfilePage *profile_page = ISKUFXCONFIG_PROFILE_PAGE(roccat_config_window_get_page(ROCCAT_CONFIG_WINDOW(window), profile_index));
	guint profile_number = profile_index + 1;
	iskufxconfig_profile_page_set_rkp(profile_page, rkp);
...
static void menu_device_reset_cb(GtkMenuItem *item, gpointer user_data) {
	...
	for (i = 0; i < ISKU_PROFILE_NUM; ++i) {
		profile_page = ISKUFXCONFIG_PROFILE_PAGE(roccat_config_window_get_page(ROCCAT_CONFIG_WINDOW(window), i));
