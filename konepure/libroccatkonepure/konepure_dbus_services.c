...
#include "roccat.h"
#include "konepure.h"
#include "konepure_dbus_plain_services.h"
...
DBusGProxy *konepure_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEPURE_DBUS_SERVER_PATH,
			KONEPURE_DBUS_SERVER_INTERFACE);
...
gboolean konepure_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepure_profile_data_changed_outside, profile_number);
}
...
gboolean konepure_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = konepure_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean konepure_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konepure_profile_changed_outside, profile_number);
}
...
gboolean konepure_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = konepure_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean konepure_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Konepure_configuration_changed_outside);
}
