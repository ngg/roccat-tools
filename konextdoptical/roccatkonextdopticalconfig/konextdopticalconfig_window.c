...
G_DEFINE_TYPE(KonextdopticalconfigWindow, konextdopticalconfig_window, ROCCAT_CONFIG_WINDOW_TYPE);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEXTDOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_get_page(roccat_window, i));
...
static void set_rmp(KonextdopticalconfigWindow *window, guint profile_index, KoneplusRmp *rmp) {
	KonextdopticalconfigProfilePage *profile_page = KONEXTDOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_get_page(ROCCAT_CONFIG_WINDOW(window), profile_index));
	guint profile_number = profile_index + 1;
	konextdopticalconfig_profile_page_set_rmp(profile_page, rmp);
...
static void menu_device_reset_cb(GtkMenuItem *item, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEXTDOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_get_page(ROCCAT_CONFIG_WINDOW(window), i));
