...
#include "konextdoptical_dbus_plain_services.h"
...
DBusGProxy *konextdoptical_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEXTDOPTICAL_DBUS_SERVER_PATH,
			KONEXTDOPTICAL_DBUS_SERVER_INTERFACE);
...
gboolean konextdoptical_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konextdoptical_profile_data_changed_outside, profile_number);
}
...
gboolean konextdoptical_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = konextdoptical_dbus_emit_profile_data_changed_outside(proxy, profile_number);
...
gboolean konextdoptical_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Konextdoptical_profile_changed_outside, profile_number);
}
...
gboolean konextdoptical_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = konextdoptical_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean konextdoptical_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Konextdoptical_configuration_changed_outside);
}
