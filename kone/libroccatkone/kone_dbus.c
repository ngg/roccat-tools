...
#include "roccat.h"
#include "kone.h"
#include "kone_dbus_plain_services.h"
...
DBusGProxy *kone_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONE_DBUS_SERVER_PATH,
			KONE_DBUS_SERVER_INTERFACE);
...
gboolean kone_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Kone_profile_data_changed_outside, profile_number);
}
...
gboolean kone_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	proxy = kone_dbus_proxy_new();
	if (!proxy)
		return FALSE;

	retval = kone_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean kone_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Kone_profile_changed_outside, profile_number);
}
...
gboolean kone_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = kone_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean kone_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Kone_configuration_changed_outside);
}
